# api/routes/user_routes.py - with lookup-client & dynamic client_id
# -*- coding: utf-8 -*-
...
def lookup_client(payload: LookupIn, db: Session = Depends(get_db)):
    ...
    row = db.execute(text("""
        SELECT client_id FROM pending_line_users
...
def register_user(payload: RegisterIn, db: Session = Depends(get_db)):
    try:
        ...
        return RegisterOut(success=True, expected_name=name, message=msg,
...
def my_cases(payload: MyCasesIn, db: Session = Depends(get_db)):
    ...
    def fmt(c): return f"• {c.case_type or '案件'} / {c.case_number or c.case_id} / 進度: {c.progress or '處理中'}"
