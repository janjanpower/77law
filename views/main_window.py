...
class MainWindow:
    ...
    def _show_case_overview(self):
        """顯示案件總覽 - 修復版本"""
        print(f"🔍 _show_case_overview 被調用")
        print(f"🔍 當前主視窗用戶資料: {getattr(self, 'user_data', 'NONE')}")
        ...
        try:
            if self.case_overview is None:
                ...
                if hasattr(self, 'selected_folder') and self.selected_folder:
                    ...
                    # 🔥 修復：使用新的方法獲取預設路徑
                    data_file = AppConfig.get_data_file_path()

                print(f"🔍 資料檔案路徑: {data_file}")
                ...
                if hasattr(self, 'user_data') and self.user_data:
                    print(f"🔍 準備傳遞用戶資料給案件總覽: {self.user_data}")
                    self.case_overview.set_user_data(self.user_data)
                    print(f"✅ 用戶資料已傳遞給案件總覽")
                else:
                    print(f"⚠️ 主視窗沒有用戶資料")
            ...
            # 先顯示總覽視窗
            print("🔍 準備顯示案件總覽視窗")
            if hasattr(self.case_overview, 'show'):
                self.case_overview.show()
            else:
                # 備用顯示方法
                self.case_overview.window.deiconify()
                self.case_overview.window.lift()
            print("✅ 案件總覽視窗已顯示")

            # 再隱藏主視窗
            self.window.after(200, self.hide)
            print("✅ 主視窗已隱藏")
            ...
            try:
                ...
                msgbox.showerror("載入失敗", f"無法載入案件總覽視窗：{str(e)}")
            except:
                pass
        except Exception as e:
            print(f"❌ 建立案件總覽時發生錯誤: {e}")
            import traceback
            traceback.print_exc()
            try:
                import tkinter.messagebox as msgbox
                msgbox.showerror("錯誤", f"建立案件總覽時發生錯誤：{str(e)}")
            except:
                pass

    def set_user_data(self, user_data):
        """設定用戶資料 - 加強版除錯"""
        print(f"🔍 MainWindow.set_user_data 被調用")
        print(f"🔍 收到的用戶資料: {user_data}")
        self.user_data = user_data
        if user_data:
            client_name = user_data.get('client_name', 'Unknown')
            client_id = user_data.get('client_id', 'Unknown')
            print(f"✅ 主視窗已設定用戶資料: {client_name} ({client_id})")
        else:
            print("⚠️ 收到的用戶資料為空")

    def on_user_login(self, user_data):
        """用戶登入事件處理 - 加強版除錯"""
        print(f"🔍 MainWindow.on_user_login 被調用")
        print(f"🔍 登入用戶資料: {user_data}")
