...
class UnifiedAdminTool:
    ...
    def _save_customers(self):
        ...
        try:
            ...
            messagebox.showerror("錯誤", f"儲存客戶資料失敗：{str(e)}")

    def _create_ui(self):
        """建立管理介面"""
        # 標題
        title_label = tk.Label(self.window, text="📋 統一授權管理工具",
                              font=("Arial", 16, "bold"))
    ...
    def _create_generate_tab(self, parent):
        """建立授權生成頁面"""
        # 客戶基本資訊
        basic_frame = tk.LabelFrame(parent, text="👤 客戶基本資訊", font=("Arial", 12))
        basic_frame.pack(pady=10, padx=20, fill='x')

        tk.Label(basic_frame, text="客戶名稱：").grid(row=0, column=0, sticky='w', padx=5, pady=5)
        self.name_entry = tk.Entry(basic_frame, width=30)
        self.name_entry.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(basic_frame, text="電子郵件：").grid(row=1, column=0, sticky='w', padx=5, pady=5)
        self.email_entry = tk.Entry(basic_frame, width=30)
        self.email_entry.grid(row=1, column=1, padx=5, pady=5)

        # 授權類型選擇
        type_frame = tk.LabelFrame(parent, text="🎫 授權類型", font=("Arial", 12))
        ...
        tk.Radiobutton(type_frame, text="🖥️🖥️🖥️ 多設備授權", variable=self.license_type,
                      ...
                      font=("Arial", 11)).grid(row=0, column=1, sticky='w', padx=10, pady=5)

        tk.Radiobutton(type_frame, text="🚀 升級授權", variable=self.license_type,
                      value="upgrade", command=self._on_type_change,
                      font=("Arial", 11)).grid(row=0, column=2, sticky='w', padx=10, pady=5)

        # 授權參數設定
        param_frame = tk.LabelFrame(parent, text="⚙️ 授權參數", font=("Arial", 12))
        param_frame.pack(pady=10, padx=20, fill='x')

        # 硬體ID（單設備和升級授權需要）
        tk.Label(param_frame, text="硬體ID：").grid(row=0, column=0, sticky='w', padx=5, pady=5)
        self.hardware_entry = tk.Entry(param_frame, width=30)
        ...
        # 設備數量（多設備和升級授權需要）
        tk.Label(param_frame, text="設備數量：").grid(row=1, column=0, sticky='w', padx=5, pady=5)
        ...
        # 授權期限
        tk.Label(param_frame, text="授權期限：").grid(row=2, column=0, sticky='w', padx=5, pady=5)
        ...
        tk.Label(param_frame, text="個月", font=("Arial", 10)).grid(row=2, column=2, sticky='w', padx=5)

        # 生成按鈕
        tk.Button(param_frame, text="🎫 生成授權碼", command=self._generate_license,
                 width=20, height=2, font=("Arial", 12),
                 bg='#4CAF50', fg='white').grid(row=3, column=0, columnspan=3, pady=20)

        # 結果顯示
        self.result_text = tk.Text(parent, height=10, width=80, font=("Arial", 10))
    ...
    def _generate_license(self):
        ...
        if not all([name, email]):
            messagebox.showerror("錯誤", "請填寫客戶名稱和電子郵件")
            return
        ...
        try:
            ...
            messagebox.showerror("錯誤", f"生成授權碼失敗：{str(e)}")
    ...
    def _generate_single_license(self, name, email):
        ...
        if not hardware_id:
            messagebox.showerror("錯誤", "單設備授權需要硬體ID")
            return None
...
📋 客戶可在最多 {max_devices} 台設備上使用此授權碼。
    ...
    def _generate_upgrade_license(self, name, email):
        ...
        if not original_hardware_id:
            messagebox.showerror("錯誤", "升級授權需要原設備的硬體ID")
            return None
...
4. 總設備數不可超過 {new_max_devices} 台
    ...
    def _copy_license(self):
        ...
        try:
            ...
            if license_lines:
                ...
                messagebox.showinfo("✅ 成功", "授權碼已複製到剪貼簿")
            else:
                messagebox.showwarning("⚠️ 提示", "未找到授權碼")
        except Exception as e:
    ...
    def _create_customer_tab(self, parent):
        """建立客戶管理頁面"""
        # 客戶列表
        list_frame = tk.LabelFrame(parent, text="👥 客戶列表", font=("Arial", 12))
        ...
        tk.Button(button_frame, text="👁️ 查看詳情", command=self._show_customer_detail,
                 width=12, font=("Arial", 10)).pack(side='left', padx=5)
    ...
    def _show_customer_detail(self):
        ...
        if not selection:
            messagebox.showwarning("⚠️ 提示", "請選擇一個客戶")
            return
...
🕐 建立時間：{customer['created_date']}
        ...
        text_widget = tk.Text(detail_window, wrap=tk.WORD, font=("Arial", 11))
        ...
        def copy_customer_license():
            ...
            messagebox.showinfo("✅ 成功", "授權碼已複製", parent=detail_window)
    ...
    def _delete_customer(self):
        ...
        if messagebox.askyesno("❓ 確認刪除", f"確定要刪除客戶 {customer_name} 的記錄嗎？"):
            ...
            self._save_customers()
            self._refresh_customer_list()
            messagebox.showinfo("✅ 成功", f"已刪除客戶 {customer_name} 的記錄")
